...
export default function UpdateStockModal({
  ...
  const { updateStock, loading } = useStock();
  ...
  useEffect(() => {
    ...
    reset({
      ...
      unitOfMeasure: stock.unitOfMeasure ?? 'piece',
      unitsPerBox: stock.unitsPerBox ?? undefined,
      itemType: stock.itemType ?? 'consumable',
      defaultPurchasePrice: stock.defaultPurchasePrice ?? undefined,
      reorderLevel: stock.reorderLevel ?? undefined,
      status: stock.status ?? 'active',
      description: stock.description ?? '',
      leadTimeDays: stock.leadTimeDays ?? undefined,
    });
  }, [isOpen, stock, getSuppliers, reset]);
        ...
        <div className="fixed inset-0 overflow-y-auto">
          <div className="flex min-h-full items-start justify-center p-4 pt-10">
            <Dialog.Panel className="w-full max-w-2xl rounded-xl bg-white p-6 shadow-xl dark:bg-gray-900">
              <Dialog.Title className="text-lg font-semibold">Edit Item</Dialog.Title>
              <form onSubmit={handleSubmit(onSubmit)} className="mt-4 space-y-4">
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <InputField label="Quantity on hand" name="quantity" type="number" registration={register('quantity')} />
