...
const STEP_FIELDS: Record<number, (keyof FarmOnboardingValues)[]> = {
  ...
  2: ['locationText'],
  3: ['size', 'grazingArea', 'housingCapacity', 'waterSource', 'veterinaryAccess', 'landmarks', 'latitude', 'longitude'],
  4: ['primaryLivestock', 'breeds', 'herdSizeEstimate'],
  5: ['contactPhone', 'contactEmail', 'emergencyContact'],
};

const STEP_ICONS = [BuildingOffice2Icon, MapPinIcon, HomeModernIcon, UserGroupIcon, PhoneIcon];
...
export default function RegisterFarmPage() {
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- fetch once when SA opens page
  }, [superAdmin]);
  ...
  const farmAdminOptions = useMemo(() => {
    ...
    return list.filter((u: any) => u?.account?.role === 'ADMIN' || u?.role === 'ADMIN');
  }, [allUsers]);
  ...
  const goNext = async () => {
    ...
    if (step === 2 && !locationString.trim()) {
      toast.error('Please select your farm location (at least country and province).');
      return;
    }
  ...
  const onSubmit = async (values: FarmOnboardingValues) => {
    try {
      let resolvedOwnerId: string | undefined;
      if (superAdmin) {
        resolvedOwnerId = ownerId || undefined;
      } else {
      ...
      const payload = buildFarmPayload(
        { ...values, locationText: values.locationText || locationString },
        resolvedOwnerId
      );
    ...
    <div className="farm-onboarding min-h-screen bg-gradient-to-br from-[#f0faf9] via-white to-[#e8f5f4] dark:from-[#0a0f0f] dark:via-[#111] dark:to-[#0d1514] font-outfit">
      <div className="max-w-4xl mx-auto px-4 py-8">
        {(superAdmin || farmAdmin) && (
          <button
            type="button"
            onClick={() => navigate(superAdmin ? '/account/farms' : '/choose-farm')}
            className="mb-4 inline-flex items-center gap-1.5 text-sm font-medium text-primary hover:underline"
          >
            <ArrowLeftIcon className="w-4 h-4" />
            {superAdmin ? 'Back to Farms' : 'Back to farm selection'}
          </button>
        )}

        <header className="text-center mb-8">
          <img src={imageSrc(Logo)} alt="DI-FARM" className="h-16 mx-auto mb-4" />
        ...
        <nav className="farm-onboarding-steps mb-8" aria-label="Registration progress">
          {ONBOARDING_STEPS.map((s, i) => {
            ...
            return (
              <div
                key={s.id}
                className={`farm-onboarding-step ${active ? 'is-active' : ''} ${done ? 'is-done' : ''}`}
              >
                ...
                <div className="hidden sm:block">
                  ...
                  <p className="text-[10px] text-gray-500 dark:text-gray-400">{s.subtitle}</p>
                </div>
              </div>
            );
          })}
        ...
        >
          {step === 1 && (
            <section className="space-y-4">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                ...
                />
                <div>
                  <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1">Farm type *</label>
              ...
              <div>
                <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1">About the farm</label>
                <textarea
          ...
          {step === 2 && (
            <section className="space-y-4">
              ...
              <p className="text-sm text-gray-500">Select country through village (Rwanda has full hierarchy).</p>
              ...
              {locationString && (
                <p className="text-sm text-primary font-medium">Selected: {locationString}</p>
              )}
              <input type="hidden" {...register('locationText')} />
              {errors.locationText && <p className="text-xs text-danger">{errors.locationText.message}</p>}
              <InputField label="Landmarks / directions" placeholder="Near main road, opposite school..." registration={register('landmarks')} name="landmarks" />
              <div className="grid md:grid-cols-2 gap-4">
                <InputField label="Latitude (optional)" placeholder="-1.9441" registration={register('latitude')} name="latitude" />
                <InputField label="Longitude (optional)" placeholder="30.0619" registration={register('longitude')} name="longitude" />
              </div>
            </section>
          )}
          ...
          {step === 3 && (
            <section className="space-y-4">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <InputField label="Total land size (sqm) *" placeholder="e.g. 5000" registration={register('size')} error={errors.size?.message} name="size" />
                <InputField label="Grazing area (sqm)" placeholder="Pasture / grazing land" registration={register('grazingArea')} name="grazingArea" />
                <InputField label="Housing capacity (animals)" placeholder="Max animals in sheds" registration={register('housingCapacity')} name="housingCapacity" />
                <div>
                  <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1">Water source</label>
                  <select {...register('waterSource')} className="form-select w-full">
                    <option value="">Select</option>
                    {WATER_SOURCES.map((w) => <option key={w} value={w}>{w}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1">Veterinary access</label>
                  <select {...register('veterinaryAccess')} className="form-select w-full">
                    <option value="">Select</option>
                    {VET_ACCESS_OPTIONS.map((v) => <option key={v} value={v}>{v}</option>)}
                  </select>
                </div>
                <div className="flex items-center gap-3 pt-6">
                  <input type="checkbox" id="hasElectricity" {...register('hasElectricity')} checked={!!hasElectricity} className="form-checkbox text-primary" />
                  <label htmlFor="hasElectricity" className="text-sm font-medium">Electricity on farm</label>
                </div>
              </div>
            </section>
          )}
          ...
          {step === 4 && (
            <section className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">Primary livestock *</label>
                ...
                {errors.primaryLivestock && <p className="text-xs text-danger mt-1">{errors.primaryLivestock.message}</p>}
              </div>
              <InputField label="Main breeds kept" placeholder="e.g. Friesian, Ankole, Crossbreed" registration={register('breeds')} name="breeds" />
              <InputField label="Estimated herd size" placeholder="Total animals on farm" registration={register('herdSizeEstimate')} name="herdSizeEstimate" />
            </section>
          )}
          ...
          {step === 5 && (
            <section className="space-y-4">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <InputField label="Farm phone *" placeholder="+250..." registration={register('contactPhone')} error={errors.contactPhone?.message} name="contactPhone" />
                <InputField label="Farm email" type="email" placeholder="farm@email.com" registration={register('contactEmail')} error={errors.contactEmail?.message} name="contactEmail" />
                <InputField label="Emergency contact" placeholder="Name & phone" registration={register('emergencyContact')} name="emergencyContact" />
              </div>
                  ...
                  >
                    ...
                    {farmAdminOptions.map((u: any) => (
                      <option key={u.id} value={u.id}>
                        {u.fullname || u.account?.username || u.account?.email || u.id}
                      </option>
                    ))}
                  </select>
                  <p className="mt-1 text-xs text-gray-500">
                    Leave empty to create an unassigned farm, then assign when adding a farm admin.
                  </p>
                </div>
              )}
              ...
              <div className="rounded-xl bg-gray-50 dark:bg-gray-900/50 border border-gray-200 dark:border-gray-700 p-4 text-sm space-y-2">
                <p className="font-semibold text-gray-800 dark:text-white">Profile summary</p>
                <p><span className="text-gray-500">Farm:</span> {formValues.name || '—'} ({formValues.type || '—'})</p>
                <p><span className="text-gray-500">Location:</span> {formValues.locationText || '—'}</p>
                <p><span className="text-gray-500">Size:</span> {formValues.size ? `${formValues.size} sqm` : '—'}</p>
                <p><span className="text-gray-500">Livestock:</span> {livestock.join(', ') || '—'}</p>
                {superAdmin ? (
                  <p className="text-xs text-success mt-2">
                    As super admin, this farm will be activated immediately after submit.
                  </p>
                ) : (
                  <p className="text-xs text-warning mt-2">
                    After submit, a super admin must activate your farm before you can manage cattle, stock, and production.
                  </p>
                )}
              </div>
            </section>
          ...
          <div className="flex justify-between items-center mt-8 pt-6 border-t border-gray-200 dark:border-gray-700">
            <button type="button" onClick={goBack} disabled={step === 1} className="btn btn-outline-primary inline-flex items-center gap-1 disabled:opacity-40">
              <ChevronLeftIcon className="w-4 h-4" /> Back
            </button>
            {step < ONBOARDING_STEPS.length ? (
              <button type="button" onClick={goNext} className="btn btn-primary inline-flex items-center gap-1">
                Next <ChevronRightIcon className="w-4 h-4" />
              </button>
            ) : (
              <button type="button" onClick={() => void handleFinalSubmit()} disabled={loading} className="btn btn-primary px-8">
                {loading ? 'Submitting…' : superAdmin ? 'Create farm' : 'Submit farm profile'}
              </button>
            )}
          </div>
        </form>
